...
async def get_book_recommendations_by_author(
    author: str,
    max_results: int = 10,
    start_index: int = 0,
) -> list[BookModel] | JSONResponse:
    """Recommend books from Google Books that match a caller-provided author."""
    author_query = author.strip()
    if not author_query:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
            detail="author must not be blank.",
        )
