...
async def get_books_by_name(query: str, max_results: int = 10, start_index: int=0):
    # Example: http://127.0.0.1:8000/api/books/?query=harry&max_results=1
    # https: // www.googleapis.com / books / v1 / volumes?q = Harry & key = AIzaSyDG5e8jG3lVTlVdMrYHqTOmDd69KkJTfo0 & maxResults = 2 & startIndex = 1
    params ={
        "q": query,
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            return JSONResponse(status_code=502, content={"error": "Upstream request to Google Books failed", "detail": str(e)})
        ...
        for item in data.get("items", []):
            ...
            try:
                ...
                return JSONResponse(status_code=422, content={"error": "Validation error", "detail": str(e)})
        return books
